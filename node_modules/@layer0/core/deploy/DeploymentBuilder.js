...
const bundle_esbuild_1 = __importDefault(require("./bundle-esbuild"));
...
class DeploymentBuilder {
    constructor(appDir = process.cwd()) {
        ...
        this.appDir = appDir;
        this.layer0Dir = path_1.join(this.appDir, paths_2.LAYER0_DIR);
        this.jsDir = path_1.join(this.appDir, paths_2.JS_DIR);
        this.staticAssetsDir = path_1.join(this.appDir, paths_2.ASSETS_DIR);
        this.permanentStaticAssetsDir = path_1.join(this.appDir, paths_2.PERMANENT_ASSETS_DIR);
        this.sourcesDir = path_1.join(this.appDir, paths_2.SOURCES_DIR);
        this.assetAliases = {};
    }
    /**
    ...
    addProdEntrypoint() {
        ...
        if (connector) {
            ...
            if (fs_extra_1.existsSync(prod)) {
                this.addJSAsset(prod, paths_3.pathForBackend(constants_1.BACKENDS.js));
            }
        }
        return this;
    }
     ...
     */
    writeFileSync(to, content, encoding = 'utf8') {
    ...
    async getRouter() {
        ...
        if (!this.router) {
            if (process.env.WEBPACK === 'true') {
                ...
                await bundle_esbuild_1.default();
            }
            const cwd = process.cwd();
            try {
                process.chdir(this.jsDir);
                this.router = loadRouter_1.default(paths_1.ROUTER_DESTINATION);
            }
            finally {
                process.chdir(cwd);
            }
    ...
    async build() {
        ...
        this.addSource(path_1.join(this.appDir, constants_1.LAYER0_CONFIG_FILE));
        ...
        this.writeFileSync(path_1.join(this.layer0Dir, 'LAYER0_VERSION'), DeploymentBuilder.layer0Version, 'utf8');
        ...
        this.writeFileSync(path_1.join(this.layer0Dir, 'layer0.json'), JSON.stringify(edgeConfig, null, '  '), 'utf8');
        if (!isEmpty_1.default(this.assetAliases)) {
            process.stdout.write('> Writing static assets aliases... ');
            this.writeFileSync(path_1.join(this.layer0Dir, constants_1.LAYER0_ASSET_ALIASES_FILE), JSON.stringify(this.assetAliases, null, '  '), 'utf8');
            process.stdout.write('done.\n');
        }
    ...
    async includeFiles() {
        if (config_1.default.has('includeFiles')) {
            ...
            for (let key in globs) {
                ...
                if (relativeDest) {
                    const files = globby_1.default.sync(path_1.join(this.appDir, key));
    ...
    writeStaticAssetExpiration() {
        this.writeFileSync(path_1.join(this.layer0Dir, 'static-asset-expiration.json'), JSON.stringify(this.assetExpiration), 'utf8');
    }
}
