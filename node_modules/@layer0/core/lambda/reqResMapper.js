...
exports.default = (event, invocation) => {
    ...
    if (event.multiValueQueryStringParameters &&
        ...
        const qs = querystring_1.stringify(event.multiValueQueryStringParameters);
        req.url += `?${qs}`;
        req.query = querystring_1.parse(qs); // we stringify and reparse to ensure that query parameters with a single value are not provided as an array
    }
 ...
 * - `wa` stands for "worker age", the number of milliseconds since the lambda instances was spawned
...
function logExecutionTime(response, invocation) {
    ...
    prefixResponseHeader(response, constants_1.HTTP_HEADERS.x0T, `wt=${invocation.duration},wc=${lambda.invocationCounter},wa=${lambda.age},wl=${lambda.lifetimeDuration}`);
}
/**
