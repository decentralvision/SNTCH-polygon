...
class ResponseWriter extends BaseResponseWriter_1.BaseResponseWriter {
    ...
    constructor(req, res, route, context) {
        ...
        this.proxyHost = async (backendConfig, { path, transformRequest, transformResponse, headers } = {}) => {
            ...
            const backend = new Backend_1.default(backendConfig);
            let pathString;
         ...
         */
        this.getAddResponseCookieAction = (name, value) => {
            return () => {
                const setCookies = this.getSetCookieValues();
                const interpolatedValue = this.interpolateParams(value);
                this.response.setHeader(constants_1.HTTP_HEADERS.setCookie, [
                    ...setCookies,
        ...
        this.addUpstreamResponseCookie = (name, value) => {
            ...
            this.scheduleExecution(this.preCacheResponseActions, this.getAddResponseCookieAction(name, value));
        };
        /**
        ...
        this.addResponseCookie = (name, value) => {
            validateWriter_1.validateCookieName(name);
            this.scheduleExecution(this.postCacheResponseActions, this.getAddResponseCookieAction(name, value));
        };
        /**
    ...
    logCacheStatus() {
        if (Cache_1.isCacheEnabled()) {
            // for now we only add x-0-t when running with --cache because the only measurement we include is oc=cached|pass|hit
