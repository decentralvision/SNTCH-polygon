...
const {
  readLayer0Version,
  ensureDomainServedByLayer0Build
...
exports.builder = _objectSpread({}, buildCommand.builder, {
  ...
  'wait-layer0': {
    type: 'boolean',
    describe: 'Flag indicating if build of your app should wait for it to be distributed to the nearest global Layer0 POP.',
    default: true
  },
  'disable-team-check': {
    type: 'boolean',
...
class DeployCommand {
  constructor(yargs) {
    _defineProperty(this, "_wrappedRun", async () => {
      ...
      if (isDirtyCommit) {
        logger.warn(`You have uncommitted changes. No commit URL will be stored in app.layer0.co.${EOL}`);
      }

      const commitUrl = isDirtyCommit ? undefined : await git.commitUrl(absolutePath, customCommitUrl);
      teamSlug = await this.checkIfSiteExistsOnAnotherTeam(siteSlug);
      ...
      const initializingStep = () => logger.step('⌛ Initializing deployment...', {
        doneTitle: 'Initializing deployment done'
      }, () => this._streamLogsUntilStatus(['initialized', 'uploading']));
      ...
      const deployingStep = () => logger.step('⌛ Deploying...', async () => {
        ...
        const build = await this._streamLogsUntilStatus(['completed']); // In local development, we usually mock the edge deployment
        // so this will never respond

        if (!isLocalDev && (!environmentIsInitialized || waitLayer0)) {
          logger.warn(`> Verifying that the deployment has reached your nearest global POP...`);
          const ARBITRARY_SERVED_TIMEOUT_MS = 5 * 60 * 1000;
          const served = await ensureDomainServedByLayer0Build(logger, build.environment.dnsDomainName, build.number, ARBITRARY_SERVED_TIMEOUT_MS);

          if (!served) {
            logger.warn('> Build has not yet propagated. If the problem persists please contact support@layer0.co');
          }
        }
      }); // Run the steps with as much parallelism as possible


      const [_initalizeStepOutput, {
        projectZipFile
      }] = await Promise.all([initializingStep(), // Packaging step requires the build
      buildStep().then(zippingStep)]);
      await uploadingStep({
        s3SelfSignedProjectPost,
        projectZipFile
      });
  ...
  _processSignalHandler(signal) {
    this._failBuild(new Layer0CLIError(`${signal} raised`)).then(() => {
      process.exit(1);
    });
  }
  /**
